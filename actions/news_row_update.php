		...
		<div class='post' id='post_<?=$row['id']?>'>
            ...
            <table width='100%' class='news_table_<?=$row['category']?>'>
                ...
                if(isset($_SESSION['authorised'])){
                    ...
                    <td>Rating: <?=((int)$row['like']-(int)$row['dislike'])?></td><td></td><td></td>
                </tr><?php
                } ?>
                <tr>
                	<td colspan='5'>
                    	<div class='commentsSlider'>
                        	<table width="100%">
                            	<?php
								$queryCom=mysql_query("SELECT * FROM `comments` WHERE `newsID`='$newsID'");
								while($rcom=mysql_fetch_array($queryCom)){?>
									<tr>
                                    	<td width="70%" style="padding:20px;"><?=$rcom['text']?></td>
                                        <td width="30%" style="text-align:right"><a>DOWN</a><span><?=(($rcom['like'])-($rcom['dislike']))?></span><a>UP</a>
                                        </td>
                                    </tr>
                                    <tr>
                                    	<td><?php 
										$userIDcom=$rcom['userID'];
										$rcomUser=mysql_fetch_array(mysql_query("SELECT * FROM `users` WHERE `id`='$userIDcom'"));
										echo $rcomUser['name'];
										 ?></td>
                                         <td style="text-align:right"><?=$rcom['date']?></td>
                                    </tr>
								<?php }?>
                            </table>
                        	<table width="100%">
                                <tr><form method="post" onSubmit="return addComent(this);" class='sendCommentForm'>
                                	<td width="90%">
                                    	<input type="hidden" id="newsIDcom" value="<?=$newsID?>">
                                        <input type="hidden" id='userIDcom' value="<?=$userID?>">
                                    	<textarea id='commentText' style='width:90%;' maxlength="200"></textarea>
                                        
                                    </td>
                                    <td width="10%" style="text-align:left">
                                    	<input type="submit" id='commentSubmit' val='add comment'>
                                    </td>
                                    </form>
                                </tr>
                            </table>
